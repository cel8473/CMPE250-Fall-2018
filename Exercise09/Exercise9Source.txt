...
;>>>>> begin main program code <<<<< 
        BL Init_UART0_IRQ    ;Initializes UART
        MOVS R1,#MAX_STRING 
...
;****************************************************************
Init_UART0_IRQ        PROC {R0-R14}					;Procs R0-R14
...
TXQ             LDR R2,=UART0_BASE
                MOVS R3,#UART0_C2_TIE_MASK
                LDRB R4,[R2,#UART0_C2_OFFSET]
                CMP R3,R0
                BEQ RXQ
                MOVS R4,#UART0_S1_TDRE_MASK       
                LDRB R5,[R2,#UART0_S1_OFFSET]
                CMP R5,R4			;if (TxInteruptEnabled) then ;TIE = 1 in UART0_C2 
                BEQ RXQ
                LDR R1,=TxQueue			; Dequeue character from TxQueue 
                BL Dequeue			
                BCS FailISR			; if (dequeue successful) then 
                STRB R4,[R2,#UART0_D_OFFSET]
RXQ             MOVS R5,#UART0_S1_RDRF_MASK
                LDRB R6,[R2,#UART0_S1_OFFSET]    ;If (RxInterrupt) then;RDRF = 1 in UART0_S1
                CMP R6,R0
                BEQ DoneISR
                LDRB R0,[R2,#UART0_D_OFFSET]
                LDR R1,=RxQueue
                BL Enqueue			; Enqueue character in RxQueue 
DoneISR         CPSIE I				;Unmask other interrupts
                POP {PC}			;Pop any registers pushed above 
FailISR         MOVS R3,#UART0_C2_T_RI			;Disable TxInterrupt 
